...
export const GET = withErrorHandling(
  async (request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    const { data: svc, error: ve } = await server
      ...
      .select(
        "id, title, short_description, pricing_type, hourly_rate, hourly_currency, fixed_amount, fixed_currency, package_label, expert_profile_id, is_published, booking_lead_minutes",
      )
      .eq("id", slot.expert_service_id)
      .maybeSingle()
    if (ve || !svc?.is_published) return createErrorResponse("Not found", 404)

    if (startMs <= Date.now() + Number(svc.booking_lead_minutes ?? 120) * 60000)
      return createErrorResponse("Slot not available", 404)
    ...
    if (holds?.length) return createErrorResponse("Slot not available", 404)
